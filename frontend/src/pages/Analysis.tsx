  ...
  const run = async () => {
    ...
    try {
      if (mode === "image") {
        ...
        const resp = await fetch("/api/analysis/video/stream", { method: "POST", body: fd, signal: ctrl.signal });
        ...
        let url = isWebcam
          ? `/api/analysis/stream/webcam?device=${rtspUrl}&preprocessing_mode=${prepMode}`
          : `/api/analysis/stream/rtsp?url=${encodeURIComponent(rtspUrl)}&preprocessing_mode=${prepMode}`;
    ...
    <DashboardLayout>
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5 animate-fade-in">
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="flex gap-2">
            <a href="/api/analysis/stream/webcam?device=0" target="_blank"
